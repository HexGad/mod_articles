<template>
  <div class="card card-body">
    <div class="container">
      <div class="row mt-3 mb-3">
        <div class="col">
          <div class="form-group">
            <label class="form-label required mb-0">Title</label>
      ...
      <div class="row mt-1 mb-3" v-if="article.slug && article.slug.length">
        <div class="col">
          <div class="form-group" v-if="edit_slug">
            <label class="form-label mb-0">Slug</label>
            <div class="input-group">
              <input class="form-control form-control-solid" v-model="article.slug" />
              <span class="input-group-text border-0 cursor-pointer" @click="edit_slug = false">
                <i class="fa fa-times text-danger"></i>
              </span>
            </div>
            <small class="text-danger" v-if="errors.slug">{{errors.slug[0]}}</small>
          </div>
          <label v-else class="ms-5">
            <span class="text-primary text-decoration-underline">{{article.slug}}</span>
            <span class="cursor-pointer mx-2" v-if="article.slug && article.slug.length" @click="edit_slug = true"> <i class="fa fa-pencil text-primary"></i></span>
          </label>
        </div>
      </div>
      ...
      <div class="row mt-3 mb-3">
        <div class="col">
          <div class="form-group">
            <label class="form-label required mb-0">Content</label>
      ...
      <div class="row mt-3 mb-3 px-3">
        <button type="button" class="btn btn-primary" @click="createArticle" :data-kt-indicator="is_loading? 'on':'off'" :disabled="is_loading">
